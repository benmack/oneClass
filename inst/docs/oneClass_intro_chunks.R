...
# <<default_oneClass_load, echo=FALSE, warning=FALSE>>=
fname <- 'oneClass_intro/oc_default.RData'
obj <- c("oc", "pred")
...
fname <- 'oneClass_intro/resamps.RData'
obj <- c("resamps")
if (!file.exists(fname)) {
  save(list=obj, file=fname)
} else {
...
fname <- 'oneClass_intro/ocUpdated.RData'
...
fname <- 'oneClass_intro/ev_modsel.RData'
