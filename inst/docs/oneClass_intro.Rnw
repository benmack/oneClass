...
setwd('D:/github/oneClass/inst/docs/')
...
\Sexpr{'train()'} function implemented in the package \pkg{caret} package \cite{Kuhn.2014}.
...
Tow of them are implemented in the function \Sexpr{'puSummary()'}.
...
<<loadpkg_eval, echo=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=4, fig.cap='The class membership (left), and the two features (middle and right) of the synthetic bananas data set.'>>=
require(oneClass)
require(caret)
require(raster)
...
\begin{multicols}{2}
<<img_y, echo=FALSE>>=
plot(bananas$y, col=c('#f4a582', '#2166ac'))
@
...
load('oneClass_intro/ev_modsel.RData')
@
...
\begin{multicols}{3}
<<evalModsel_puF, warning=FALSE, message=FALSE, echo=FALSE>>=
par(cex=1.75, mgp=c(1.5, .5, 0), mar=c(3, 3, .1, .1))
plot(evList$puF, evList$mxK.K, xlab="puF", ylab="max. kappa")
@

<<evalModsel_puAuc, warning=FALSE, message=FALSE, echo=FALSE>>=
par(cex=1.75, mgp=c(1.5, .5, 0), mar=c(3, 3, .1, .1))
plot(evList$puAuc, evList$mxK.K, xlab="puAuc", ylab="max. kappa")
@

<<evalModsel_tpr, warning=FALSE, message=FALSE, echo=FALSE>>=
par(cex=1.75, mgp=c(1.5, .5, 0), mar=c(3, 3, .1, .1))
plot(evList$puPpv, evList$Tpr, xlab="probability of positive prediction", ylab="true positive rate")
@
\end{multicols}
...
If the \pkg{spatial.tools} is installed \Sexpr{'rasterEngine()'} will be used for 
prediction. 
...
If some areas are masked out with the \Sexpr{'mask'} argument the computational cost is 
reduced because the masked pixels will not be processed. 

Note that with the \Sexpr{'rasterEngine()'} the masked pixels are still loaded from the file, they will just not be predicted. 
