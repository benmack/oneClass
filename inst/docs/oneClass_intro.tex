...
The purpose of \textbf{one-class classifier} is to classify new data based on a 
...
It can be installed in R with the package \code{devtools} and the following commands:
...
{\centering \includegraphics[width=\maxwidth]{figures/plots-loadpkg_prelim} 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# index <- createResamplePu( tr.y, times = 25, index.indep = 276:500)}
\hlkwd{set.seed}\hlstd{(seed)}
...
\hlstd{pred} \hlkwb{<-} \hlkwd{readAll}\hlstd{(pred)}
\end{alltt}
\end{kframe}
\end{knitrout}
