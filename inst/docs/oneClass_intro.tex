...
\begin{alltt}
\hlkwd{setwd}\hlstd{(}\hlstr{"inst/docs/"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: kann Arbeitsverzeichnis nicht wechseln}}\end{kframe}
\end{knitrout}
...
The purpose of \textbf{one-class classifier} is identical to the to classify new data based on a classification model learned from labeled examples.
...
Some words about \hlstd{puAuc}, \hlstd{puF}, \hlstd{Tpr}, and \hlstd{puPpv} ...
...
{\centering \includegraphics[width=\maxwidth]{figures/plots-loadpkg} 

}

\caption[The class membership (left), and the two features (middle and right) of the synthetic bananas data set]{The class membership (left), and the two features (middle and right) of the synthetic bananas data set.\label{fig:loadpkg}}
\end{figure}
