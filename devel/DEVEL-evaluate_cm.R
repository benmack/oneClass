...
set.seed (seed)
te.i <- sample ( ncell (mixsim$y), 1000)
te.x <- extract (mixsim$x, te.i)
te.y <- extract (mixsim$y, te.i)
...
oc <- trainOcc ( x = tr.x, y = tr.y, tuneGrid=tuneGrid, trControl = trControl )
...
plot_thDepPPPvsTPR <- function (x) {
  
  hop <- holdOutPredictions(x, aggregate=TRUE)
  n.pos <- length(hop$pos)
  n.un <- length(hop$un)
  
  th <- quantile(hop$pos, (1:100)/100)
  rtrn <- sapply(th, function(th) c(th=th, 
                                    tpr=sum(hop$pos>=th)/n.pos, 
                                    ppp=sum(hop$un>=th)/n.un ))
  
  
  rtrn <- data.frame()
  
  rng <- range(c(hop$pos, hop$un))
  
  
  
  
  ppp <- quantile(hop$un, (1:100)/100)
...
pred <- predict(oc, mixsim$x)
plot(pred)
### ----------------------------------------------------------------------------
### prediction of a raster object with a mask (this saves prediction, but 
### not loading the data from the file)
pred <- predict(oc, mixsim$x, mask=mask)
plot(pred)
### ----------------------------------------------------------------------------
### tiledRaster
### ...
require(rasterTiled)
ut <- rasterTiled(mixsim$x, nTiles=4)
utm <- rasterTiled(mixsim$x, mask=mask, nTiles=4)
...
### ...
system.time(pred <- predict(oc, mixsim$x))
system.time(pred <- predict(oc, mixsim$x, mask=mask))
